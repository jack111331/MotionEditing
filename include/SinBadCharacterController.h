...
private:
    ...
    void updateBody(Real deltaTime)
    {
        ...
        if (mTestKey) {
            auto skeleton = mBodyEnt->getSkeleton();
//            auto boneList =skeleton->getBones();
//            for (auto i = boneList.begin();i != boneList.end();++i) {
//                auto bone = (*i);
//                auto boneName = bone->getName();
//                std::cout << "Bone Name: " << boneName << std::endl;
//            }
            auto bone = skeleton->getBone("Thigh.R");
            bone->reset();
            bone->setManuallyControlled(true);

            unsigned short boneHandle = bone->getHandle();
            Ogre::AnimationStateIterator animStateIt = mBodyEnt->getAllAnimationStates()->getAnimationStateIterator();
            ...
            while( animStateIt.hasMoreElements() )
            {
                ...
                // get animation
                Ogre::Animation *pAnimation = skeleton->getAnimation( pAnimState->getAnimationName() );
                if( !pAnimation->hasNodeTrack( boneHandle ) )
                    continue;

                Ogre::NodeAnimationTrack *pNodeAnimTrack = pAnimation->getNodeTrack( boneHandle );

                Ogre::Real time = pAnimState->getTimePosition();
                Ogre::Real weight = pAnimState->getWeight();

                // apply animation to bone
                
                pNodeAnimTrack->applyToNode( bone, time, weight );
            }
